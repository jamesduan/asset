...
def handleImpl():
    ...
    newdataids = ConfigInfo.objects.filter(env=6)
    newgroups = ConfigGroup.objects.filter(status=1)
    newgroupstatuses = ConfigGroupStatus.objects.filter(status=0)

    oldDatas = getOldConfigInfos(olddataids, oldgroups)
    newDatas = getNewConfigInfos(newdataids, newgroups, newgroupstatuses)
    return cmpConfigInfos(newDatas, oldDatas)
