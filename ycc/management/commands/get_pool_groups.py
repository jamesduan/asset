...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if hostnum > 0:
            for hostip in args:
                output = ssh('ls /var/www/webapps/config/ycc/snapshot/', hostip, user='whtest')[2]
                output = output.split('\n')
                selgroups.extend(output)
                print len(output)
        print len(selgroups)
        selgroups = list(set(selgroups))
        print len(selgroups)
        importGroupDatas(selgroups)
...
def importGroupDatas(selgroups):
    ...
    oldgroups = OldConfigGroup.objects.using('configcentre').filter(group_id__in=selgroups)
